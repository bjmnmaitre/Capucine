...
export interface ConversationSession {
  ...
  /**
   * Usage context accumulated across the conversation — "pour écouter de la
   * musique" on turn 2, "surtout dans les transports" on turn 3.
   *
   * WHY IT LIVES ON THE SESSION AND NOT ON THE PROFILE
   * ──────────────────────────────────────────────────
   * This is what the user wants FOR THIS SEARCH. "Je prends le train demain"
   * must not quietly become a permanent preference (spec §8). The session
   * expires; UserProfile is never written to from here. A usage that should
   * become permanent has to go through the profile's own explicit update path.
   *
   * Turns 2+ supply criteria via preInterpretedCriteria and skip
   * interpretation entirely, so without this field a usage stated mid-
   * conversation would be lost on the very next turn.
   */
...
export class ConversationManager {
  ...
  private buildSession(
    ...
    // Seeded from the first turn's interpretation, refined by applyFollowUp().
    const usageContext = result.interpretedRequest?.usageContext;
  ...
  applyFollowUp(
    ...
    options: {
      ...
      /** Usage context stated on THIS turn — merged, never overwritten. */
      usageContext?: UsageContext;
    } = {}
  ): ApplyFollowUpResult {
    const session = this.getSession(sessionId, userId);
    ...
    // usageContext: MERGED, never replaced — "pour la musique" on turn 2 then
    // "surtout dans les transports" on turn 3 must yield both, with the newest
    // statement dominant. Still session-scoped: nothing is written to the
    // permanent profile (spec §8).
    const usageContext = mergeUsageContexts(session.usageContext, options.usageContext);
